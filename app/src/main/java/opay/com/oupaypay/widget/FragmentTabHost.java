package opay.com.oupaypay.widget;
...
public class FragmentTabHost extends TabHost implements
    ...
    static final class TabInfo {
        ...
        private int currentTabId;   //增加的一个字段  用来判断当前的显示fragment的id

        TabInfo(String _tag, Class<?> _class, Bundle _args, int id) {
            tag = _tag;
            clss = _class;
            args = _args;
            currentTabId = id;
    ...
    public void addTab(TabSpec tabSpec, Class<?> clss, Bundle args, int i) {
        tabSpec.setContent(new DummyTabFactory(mContext));
        String tag = tabSpec.getTag();

        TabInfo info = new TabInfo(tag, clss, args, i);
        ...
        if (mLastTab != newTab) {
            ...
            if (mLastTab != null) {
                if (mLastTab.fragment != null) {
                    //对tab增加一个id 字段 用来根据id来判断  是走从左到有的动画还是从右到左的动画
                    if (mLastTab.currentTabId <newTab.currentTabId) {
                        //执行动画  右 -- > 左
                        ft.setCustomAnimations(R.anim.push_right_in, R.anim.push_left_out);
                    } else {
                        //执行动画  左 -- > 右
